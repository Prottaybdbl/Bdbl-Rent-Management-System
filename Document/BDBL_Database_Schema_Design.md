# বিডিবিএল ভাড়া ব্যবস্থাপনা সিস্টেম - ডেটাবেস স্কিমা ডিজাইন

## ডেটাবেস টেবিল স্ট্রাকচার
...
CREATE TABLE buildings (
    ...
    ownership_type VARCHAR(50), -- নিজস্ব, ভাড়া, লিজ
...
### ৫. Tenant/Company (ভাড়াটিয়া/কোম্পানি)
```sql
...
### ৬. Contact Person (যোগাযোগ ব্যক্তি)
```sql
...
### ৭. Lease Agreement (ভাড়া চুক্তি)
```sql
...
### ৮. Agreement Units (চুক্তির ইউনিট - অনেক-থেকে-অনেক সম্পর্ক)
```sql
...
### ৯. Agreement Parking (চুক্তির পার্কিং)
```sql
...
### ১০. Agreement Documents (চুক্তির নথিপত্র)
```sql
...
### ১১. Utility Types (ইউটিলিটির ধরন)
```sql
...
### ১২. Utility Rate Slabs (ইউটিলিটি রেট স্ল্যাব)
```sql
...
### ১৩. Utility Meters (ইউটিলিটি মিটার)
```sql
...
### ১৮. Payment Allocations (পেমেন্ট বরাদ্দ)
```sql
...
### ২০. Advance Payments (অগ্রিম পেমেন্ট)
```sql
...
### ২১. Advance Adjustments (অগ্রিম সমন্বয়)
```sql
...
### ২৬. System Settings (সিস্টেম সেটিংস)
```sql
...
### ২৭. Reports (রিপোর্ট স্ক্রিপ্ট সেভ করার জন্য - ঐচ্ছিক)
```sql
...
## ইন্ডেক্স (Index) তৈরি - পারফরমেন্সের জন্য
...
## ভিউ (Views) - সহজে ডেটা অ্যাক্সেস করার জন্য

### ১. বর্তমান সক্রিয় চুক্তি ভিউ
```sql
...
## ট্রিগার (Triggers) - স্বয়ংক্রিয় প্রসেস

### ১. updated_at ট্রিগার (সকল টেবিলের জন্য)
```sql
...
### ২. ইউনিট স্ট্যাটাস আপডেট ট্রিগার
```sql
...
### ৩. মোট মাসিক ভাড়া ক্যালকুলেশন ট্রিগার
```sql
...
## স্টোরড প্রসিজার (Stored Procedures) - জটিল অপারেশনের জন্য

### ১. মাসিক ভাড়া বিল জেনারেট
```sql
...
এই ডেটাবেস স্কিমা আপনার সিস্টেমের সম্পূর্ণ ডেটা মডেল প্রদান করে। এটি নরমালাইজড, স্কেলেবল এবং পারফরমেন্ট।
